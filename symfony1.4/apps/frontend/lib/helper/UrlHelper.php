...
function link_to1($name, $internal_uri, $options = array())
{
  ...
  if (isset($html_options['query_string']))
  {
    ...
    $html_options['href'] .= BridgeTools::getSortOrderSeparator().$html_options['query_string'];
...
{
	$uri = sfContext::getInstance()->getController()->genUrl($internal_uri, $absolute);

	$uri = BridgeTools::makeUriJoomlaCompatible($uri);
